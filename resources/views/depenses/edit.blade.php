...
@section('actions')
    @if(auth()->user()->canUnlockFinancialScope($depense->service_scope))
        <form method="POST" action="{{ route('depenses.unlock', ['depense' => $depense, 'module' => $module->value]) }}" class="form-inline unlock-controls">
            @csrf
...
@section('content')
    <div class="grid-2">
        <div class="panel">
            <form method="POST" action="{{ route('depenses.update', ['depense' => $depense, 'module' => $module->value]) }}" enctype="multipart/form-data" class="stack-md">
                @csrf
                @method('PUT')

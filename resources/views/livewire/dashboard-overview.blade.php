@php
    $trend = collect($this->metrics['trend']);
    ...
    $topRecettes = $this->metrics['top_recettes'];
    $topDepenses = $this->metrics['top_depenses'];
    $topSaisie = $this->metrics['top_saisie'];
@endphp
...
<div class="stack-lg">
    ...
    <div class="stats-grid">
        <x-stat-card title="Total recettes" :value="number_format($this->metrics['recettes_total'], 0, ',', ' ') . ' FCFA'" :meta="$this->metrics['recettes_count'] . ' enregistrements'" icon="wallet" />
        <x-stat-card title="Total dépenses" :value="number_format($this->metrics['depenses_total'], 0, ',', ' ') . ' FCFA'" :meta="$this->metrics['depenses_count'] . ' enregistrements'" icon="receipt" />
        <x-stat-card title="Total versements" :value="number_format($this->metrics['versements_total'], 0, ',', ' ') . ' FCFA'" :meta="$this->metrics['versements_count'] . ' enregistrements'" icon="bank" />
    </div>

    @if ($this->metrics['missing_yesterday']->isNotEmpty())
        <div class="alert alert-error alert-rich">
            ...
            <div>
                ...
                <ul>
                    @foreach ($this->metrics['missing_yesterday'] as $control)
    ...
    <div class="grid-2">
        <div class="panel">
            <div class="panel-header">
                <div>
                    ...
                    <p class="text-muted">Courbes dynamiques sur la période filtrée</p>
                </div>
            </div>
        ...
        <div class="panel">
            <div class="panel-header">
                <div>
                    <h2>{{ $this->metrics['user_can_view_all'] ? 'Dernières notifications' : 'Mes notifications récentes' }}</h2>
                    <p class="text-muted">Anomalies, rappels et contrôles journaliers</p>
                </div>
            </div>
            <div class="notification-list">
                @forelse ($this->metrics['recent_notifications'] as $notification)
    ...
    @if ($this->metrics['user_can_view_all'])
        <div class="grid-2">
            <div class="panel">
                <div class="panel-header">
                    <div>
    ...
    @else
        <div class="panel">
            <div class="panel-header">
                <div>
                    <h2>Dernières opérations de votre périmètre</h2>
                    <p class="text-muted">Vue pratique pour la gare ou les gares qui vous sont affectées</p>
                </div>
            </div>
            <div class="table-wrapper table-plain">
                <table>
                    ...
                    <tbody>
                        @forelse ($this->metrics['recent_operations'] as $row)
                            <tr>
                                <td>{{ $row['type'] }}</td>
                                <td>{{ $row['date'] }}</td>
                                <td>{{ $row['gare'] }}</td>
                                <td>{{ number_format($row['amount'], 0, ',', ' ') }} FCFA</td>
                            </tr>
                        @empty
                            <tr><td colspan="4">Aucune opération trouvée.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="panel">
        ...
        <div class="table-wrapper table-plain">
            <table>
                ...
                <tbody>
                    @forelse ($this->metrics['controls'] as $control)
                        <tr>

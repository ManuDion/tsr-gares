...
class PerformanceReportController extends Controller
{
    public function __invoke(Request $request): View
    {
        ...
        $topSaisie = Gare::query()
            ->withCount([
                'recettes as recettes_count' => fn ($q) => $q->whereBetween('operation_date', [$startDate, $endDate]),
                'depenses as depenses_count' => fn ($q) => $q->whereBetween('operation_date', [$startDate, $endDate]),
                'versementsBancaires as versements_count' => fn ($q) => $q->whereBetween('operation_date', [$startDate, $endDate]),
            ])
            ->get()
            ->map(function ($gare) {
        ...
        return view('reports.performance', compact('topSaisie', 'topRecettes', 'topDepenses', 'startDate', 'endDate'));
    }
}
